...
public class RedisUtil {
    ...
    private final Logger logger = LoggerFactory.getLogger(getClass());//日志输出类
    ...
    public void delByPrex(String prex) {
        Set<String> keys = this.keys(prex);
    ...
    public long sSet(String key, Object... values) {
        try {
            ...
            for (int i = 0; i < values.length; i++) {
                if (ToolUtil.isNotEmpty(values[i])) {
                    list.add(values[i]);
                }
            }
     ...
     * @return
     */
    public boolean lSetList(String key, Collection<?> values) {
        return this.lSetList(key, values, -1);
    }
     ...
     * @return
     */
    public boolean lSetList(String key, Collection<?> values, long time) {
        try {
